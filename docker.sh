...
EOF
        ...
        log_info "已更新Debian源配置为标准格式"
        
        # 清理并更新
        sudo apt clean
        sudo apt update --fix-missing -y || {
            log_warn "标准源更新失败，尝试使用阿里云镜像源..."
            
            # 使用阿里云镜像源作为备选
            cat <<EOF | sudo tee /etc/apt/sources.list > /dev/null
# 阿里云 Debian $VERSION_CODENAME 镜像源
...
deb http://mirrors.aliyun.com/debian/ $VERSION_CODENAME-updates main non-free contrib
deb-src http://mirrors.aliyun.com/debian/ $VERSION_CODENAME-updates main non-free contrib
