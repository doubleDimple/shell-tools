...
run_k8s(){
    if [[ $is_master -eq 1 ]]; then
        # kubeadm init（不再强制指定 --kubernetes-version，让 kubeadm 自己选 compatible 版本）
        if [[ $network == "flannel" ]]; then
            run_command "kubeadm init --pod-network-cidr=10.244.0.0/16"
            ...
            # 按 flannel 官方文档使用新地址 :contentReference[oaicite:2]{index=2}
